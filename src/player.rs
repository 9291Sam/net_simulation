...
impl Player {
    pub fn new(id: usize, env: &Environment) -> Self {
        ...
        Self {
            ...
            active_path: Vec::new(),
            // Quick stagger at the very start of the simulation (0 to 2 seconds)
    ...
    pub fn movement_update(&mut self, env: &Environment) {
        ...
        if self.active_path.is_empty() {
            ...
            // Fallback: if the path failed to generate, wait 1 second and try again next tick
            if self.active_path.is_empty() {
        ...
        if distance <= AGENT_SPEED_UNITS_PER_TICK {
            ...
            // FIX: Assign the long indoor wait timer ONLY upon arriving at the final destination
            if self.active_path.is_empty() {
